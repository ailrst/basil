...
// see this branch https://github.com/ailrst/basil/blob/offline-lifter/src/main/scala/translating/hackygtirb.scala
...
def lgtirb(mods: Seq[Module], cfg: CFG, mainAddress: Int): Program = {
      ...
      b.value.code match {
        case Some(c) =>
          BlockPos(
            ...
            b.offset,
            c.size,
            cont.substring(b.offset.toInt, (b.offset + c.size).toInt)
          )
      }
    )
  )
  ...
  val opsize = 4
  val opcodes = codeblocks.map(b =>
    (
      b,
      Range
        .Exclusive(0, b.size.toInt, opsize)
  ...
  def liftBlock(label: String, ops: Iterable[Int], addr: Long) = {
    var address = addr
    var liftState = BasilLiftState(label)
    ops.foreach(op =>
      // liftState.implicit_set_pc(address, Some(s"opcode ${op.toHexString}"))
      address += 32

      try {
        Lifter.liftOpcode(op, address, liftState)
      } catch {
        case e: Exception => {
          if (op != 0xd503201f) then
            Logger.warn(
              s"Disassembly error on ${op.toHexString} (using NOP) : $e\n${e.getStackTrace.map(_.toString).mkString("\n  ")} "
            )
        ...
        case e: NotImplementedError => {
          if (op != 0xd503201f) then
            Logger.warn(
              s"Disassembly not implemented ${op.toHexString} (using NOP) : $e\n  ${e.getStackTrace.map(_.toString).mkString("\n  ")}"
            )
          liftState.push_stmt(NOP(Some(s"Potentially mis-lifted from ${op.toHexString}")))
        }
      }
  ...
  val procedure_names =
    functionNames.map((functionUUID, symbolUUID) => (functionEntries(functionUUID).head, symbol_names(symbolUUID)))
  val procedures =
    proxies.map((u, f) => u -> f.entry).toMap ++ procedure_names.map((fuid, _) => fuid -> allblocks(fuid).entry).toMap

  val verts_not_blocks = cfg.vertices.toSet.diff(allblocks.keySet)
  assert(verts_not_blocks.isEmpty)

  val conds = cfg.vertices
    .map(v => {
  ...
  def get_pc(binfo: BasilLiftState): (String, BranchInfo) = {
  ...
  for ((b, es) <- gedges) {
    ...
    edge.label match {
      ...
      case Some(EdgeLabel(false, true, Type_Branch, _)) if (procedures.keySet.contains(edge.targetUuid)) => {
        val tgt: String = procedures(edge.targetUuid)
        sb.add_call(dsl.directCall(tgt), ft_tb.map(_.entry))
        ft_tb.foreach(sb.merge_state)
      }
      case Some(EdgeLabel(false, false, Type_Branch, _)) if (procedures.keySet.contains(edge.targetUuid)) =>
        sb.add_call(dsl.indirectCall(procedures(edge.targetUuid), ft_tb.map(_.entry))); ft_tb.foreach(sb.merge_state)
      case Some(EdgeLabel(false, true, Type_Branch, _)) => sb.add_goto(tb.entry); sb.merge_state(tb)
      case Some(EdgeLabel(false, true, Type_Call, _)) =>
        sb.add_call(dsl.directCall(procedures(edge.targetUuid)), ft_tb.map(_.entry))
        ft_tb.foreach(sb.merge_state)
  ...
  class CleanupVis extends Visitor {
    override def visitBlock(node: ir.Block): ir.Block = {
      for (s: Statement <- node.statements) {
        s match {
          case LocalAssign(l: Variable, r, _) if (l.name) == "BTypeNext" || (l.name) == "__BranchTaken" =>
            node.statements.remove(s)
          case _ => ()
        }
      }
      visitJump(node.jump)
  ...
  var p = dsl.prog(real_procedures.map((fname, blocks) => dsl.proc(fname, blocks.toIR(): _*)).toList)
