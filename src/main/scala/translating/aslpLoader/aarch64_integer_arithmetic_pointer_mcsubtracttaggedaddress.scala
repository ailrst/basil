...
def f_aarch64_integer_arithmetic_pointer_mcsubtracttaggedaddress (v_st: LiftState,v_enc: BV) : Unit = {
  ...
  if (f_eq_bits(v_st, BigInt(5), bvextract(v_st,v_enc,BigInt(16),BigInt(5)), mkBits(v_st, 5, BigInt("11111", 2)))) then {
