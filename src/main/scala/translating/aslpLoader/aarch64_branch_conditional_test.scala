...
def f_aarch64_branch_conditional_test (v_st: LiftState,v_enc: BV) : Unit = {
  if (f_eq_bits(v_st, BigInt(1), bvextract(v_st,v_enc,BigInt(31),BigInt(1)), mkBits(v_st, 1, BigInt("1", 2)))) then {
    ...
    f_gen_store (v_st,v__PC.v,f_gen_add_bits(v_st, BigInt(64), f_gen_load(v_st, v__PC.v), f_gen_bit_lit(v_st, BigInt(64), f_SignExtend(v_st, BigInt(16), BigInt(64), f_append_bits(v_st, BigInt(14), BigInt(2), bvextract(v_st,v_enc,BigInt(5),BigInt(14)), mkBits(v_st, 2, BigInt("00", 2))), BigInt(64)))))
