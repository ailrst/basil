...
class SystemTestsOfflineLifter extends AnyFunSuite {
  ...
  def runTest(path: String, name: String, variation: String, shouldVerify: Boolean, useOfflineLifter: Boolean): Unit = {
    val directoryPath = path + "/" + name + "/"
    val variationPath = directoryPath + variation + "/" + name
    val specPath = directoryPath + name + ".spec"
    val outPath = if useOfflineLifter then variationPath + "_offline.bpl" else variationPath + "_online.bpl"
    val inputPath = variationPath + ".gts"
    ...
    val expectedOutPath = if useOfflineLifter then variationPath + "_offline.expected" else variationPath + "_online.expected"
