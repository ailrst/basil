...
L: c -> true, buf -> (c != 0bv32) , stext -> false
Rely: c == old(c), old(Gamma_buf) ==> Gamma_buf, stext==old(stext), buf==old(buf), DIRECT :: "mem == old(mem)" : bool, DIRECT:: "Gamma_mem == old(Gamma_mem)" : bool // doesn't translate buf=old(buf) correcly, only constrains first byte :(
...
Subroutine: mfree
Requires: DIRECT :: "(forall i :bv64:: (bvuge64(i, R0) && bvult64(i,  12bv64)) ==> Gamma_mem[i])" : bool
//Ensures DIRECT: "(forall i: bv64 :: stack[i] == old(stack[i]))"
//Ensures DIRECT: "(forall i: bv64 :: Gamma_stack[i] == old(Gamma_stack[i]))"
...
Subroutine: strlen
  Modifies: R0
  Requires DIRECT: "(exists i: bv64 :: (bvule64((R0), i)) && (bvult64(i, bvadd64((R0), 12bv64))) && mem[i] == 0bv8)"
